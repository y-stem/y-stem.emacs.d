...
(let ((class '((class color) (min-colors 89)))
  ...
  (custom-theme-set-faces
   ...
   `(outline-2 ((,class (:foreground ,purple :weight bold :height 1.2 :bold nil))))
   `(outline-3 ((,class (:foreground ,peach :weight bold :height 1.2 :bold nil))))
   `(outline-4 ((,class (:foreground ,green-light :weight bold :height 1.2 :bold nil))))
   `(outline-5 ((,class (:foreground ,blue :weight bold :height 1.2 :bold nil))))
   `(outline-6 ((,class (:foreground ,purple :weight bold :height 1.2 :bold nil))))
...
Supposedly makes text with unicode characters not bog down.
...
...so SML indentation doesn't move around during heading
promotion/demotion. Otherwise, just do a "select all" C-x h, then M-x untabify
...
#+begin_src emacs-lisp
(load-file "~/.emacs.d/modes/setup-sml.el")
;(load-file "~/.emacs.d/modes/sml-mode-6.10.el")
;(load-file "~/.emacs.d/modes/ob-sml.el")
;(require 'sml-mode)
;(require 'ob-sml)
(use-package ob-sml
   :defer t
...
(use-package org
  ...
  ;;file to save todo items (this gets overridden by my-custom.el...
  ;; ...so we'll retire it
  ;; (setq org-agenda-files (quote ("~/Dropbox/org/ccynata/ccynata-agenda.org")))
  ...
  ;; This is being overridden by my-custom.el...
  ;;...so we'll retire it...
  ...
  (setq org-todo-keyword-faces
	'(("TODO" . (:foreground "red4"))
	  ("NEXT" :foreground "blue3")
	  ("TOGROK" :foreground "cyan4")
	  ("GROKKING" :foreground "cyan3")
	  ("GROKKED" :foreground "PaleGreen4")
	  ("IN-PROGRESS" :foreground "light blue")
	  ("WAITING" :foreground "orange")
	  ("FEEDBACK" :foreground "DarkGoldenrod4")
	  ("HOLD" :foreground "DarkSlateGray2")
	  ("MEETING" :foreground "DarkSlateGray2")
	  ("INCIDENT" :foreground "MediumPurple4")
	  ("VERIFY" :foreground "MediumPurple2")
	  ("CANCELLED" :foreground "PaleGreen4")
	  ("FIXED" :foreground "PaleGreen4")
	  ("TRANSFERRED" :foreground "PaleGreen3")
	  ("FEDBACK" :foreground "PaleGreen3")
	  ("VERIFIED" :foreground "PaleGreen3")
	  ("DONE" :foreground "PaleGreen4")))

  (setq org-enforce-todo-dependencies t)


  ;; This will prompt ()with completion enabled) the heading path where
  ;; you'd like to put the capture template-created header. 
...
(use-package org-ref
  ...
  (setq org-ref-bibliography-notes "~/Dropbox/org/biblio/notes/notes.org"
        org-ref-default-bibliography '("~/Dropbox/org/biblio/ref.bib")
        org-ref-pdf-directory "~/Dropbox/org/biblio/bibtex-pdfs/")

  (unless (file-exists-p org-ref-pdf-directory)
    (make-directory org-ref-pdf-directory t))

  (setq helm-bibtex-bibliography "~/Dropbox/org/biblio/ref.bib")
  (setq helm-bibtex-library-path "~/Dropbox/org/biblio/bibtex-pdfs/")

  (setq helm-bibtex-pdf-open-function
        (lambda (fpath)
          (start-process "open" "*open*" "open" fpath)))

  (setq helm-bibtex-notes-path "~/Dropbox/org/biblio/notes/notes.org")
...
(tempo-define-template "org-src-haskell-basic"
                       '("#+begin_src haskell :results" n
                         n "#+end_src" )
                       "<hb" "Insert haskell basic block" 'org-tempo-tags)

(tempo-define-template "org-src-sml-basic"
                       '("#+begin_src sml :exports both" n
                         n "#+end_src" )
                       "<ml" "Insert SML basic block" 'org-tempo-tags)
...
(tempo-define-template "org-sharp-s"
		       '("ß")
		       "<sz" "Insert sharp-s" 'org-tempo-tags)

(tempo-define-template "org-right-arrow"
		       '("➝")
		       "<ra" "Insert right arrow" 'org-tempo-tags)

#+end_src

#+RESULTS:
: tempo-template-org-right-arrow
...
*** Go to next buffer
:PROPERTIES:
:TIME: 2020-03-15T23:54:22
:ID: 1a50322f-d2f8-48df-a5cf-f569d2bf9e43
:EDGES:  
:END:
...
*** Switch to *scratch* 
:PROPERTIES:
:TIME: 2020-03-15T23:28:34
:ID: 56a96062-3b75-4837-a34b-aa6f0da54fdd
:EDGES: [[file:~/Dropbox/org/Bernt Hansen/org-mode.org::#KeyBindings][Key bindings]] 
:END:
